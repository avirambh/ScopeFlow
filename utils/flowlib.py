...
def visualize_flow(flow, mode='Y', subplote=False, occ=None, enhance=True):
    """
    ...
    if mode == 'Y':
        ...
        img = flow_to_image(flow)
        if occ is not None:
            img = (img * (1-occ)).astype(np.uint8)

        if enhance:
            img = ImageEnhance.Brightness(Image.fromarray(img)).enhance(0.8)
        ...
        factor = 8
        img[:, :, 1] = np.sqrt(du * du + dv * dv) * factor / max_flow
        # phase layer
        img[:, :, 2] = factor - img[:, :, 1]
        # clip to [0,1]
        small_idx = img[:, :, 0:3] < 0
